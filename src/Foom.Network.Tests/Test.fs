...
type Test() = 
    ...
    member this.ReliableSequenced () =
        let senderPacketPool = PacketPool 2048
        let receiverPacketPool = PacketPool 2048

        let mutable valueToCheck = 0uy

        let receiver = Receiver.createReliableSequenced receiverPacketPool (fun ack -> ()) (fun packet -> valueToCheck <- packet.Raw.[packet.Length - 1])

        let mutable canSimulatePacketLoss = true

        let sender = 
            Sender.createReliableSequenced senderPacketPool (fun packet ->
                let receivePacket = receiverPacketPool.Get ()
                packet.CopyTo receivePacket
                ...
                if canSimulatePacketLoss then
                    if int packet.SequenceId % 2 = 0 then
                        ...
                        receiverPacketPool.Recycle receivePacket
                else
                    receiver.Send receivePacket
            )
