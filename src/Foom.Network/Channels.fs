...
type NewSender (packetPool : PacketPool, output) =
    ...
    interface ISender with
        ...
        member __.Process f =
            for i = 0 to outputQueue.Count - 1 do
                let packet = outputQueue.[i]
                f packet
...
[<Sealed>]
type UnreliableSender (packetPool) =
    ...
    let sender = sender :> ISender

    let recycle = packetPool.Recycle
    ...
    interface ISender with
        ...
        member __.Reset () =
            sender.Process recycle
            sender.Reset ()

//type ReliableOrderedAckSender (packetPool) =



//    interface IAckSender with

//        member __.Enqueue ack =
