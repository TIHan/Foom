...
module WallPart =
    ...
    let newUpdateUV (vertices: Vector3 []) (uv: Vector2 []) width height (part: WallPart) =
    ...
    let newCreateUV (vertices: Vector3 []) width height (part: WallPart) =

        let uv = Array.zeroCreate vertices.Length

        newUpdateUV vertices uv width height part
